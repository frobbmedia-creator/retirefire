# Hermes Max — RetireFire Reddit + Facebook + LinkedIn

**Standing job. Start with `GO.txt` then `DAY1_EXECUTE.md`.**

| File | What |
|------|------|
| `GO.txt` | One-screen start order |
| `DAY1_EXECUTE.md` | **Run this first session** |
| `LINKEDIN_UPDATE.md` | Company (all builds) then short $5k offer — do not mash |
| `FROBB_MEDIA_OFFER.md` | $5k/mo model. Context = one line in “also a Frobb creation.” |
| `MAX_READ_THIS_FIRST.md` | Accounts, rules, daily cadence |
| `TARGETS.md` | Subreddits + Facebook groups |
| `POSTS_READY.md` | Copy to rewrite 20%+ before posting |
| `LOG.csv` | Session log template |

Identities:

- Facebook groups: **Frank Peake** personal profile (not the Boca Skin Page)
- Reddit: **frobbmedia@gmail.com** (existing Reddit account — never a second user)
- X: **Frankpeake1** (RetireFire posts; do not mix Orblings)
- LinkedIn person: https://www.linkedin.com/in/frank-peake-09980411
- LinkedIn brand: https://www.linkedin.com/in/frobb-media-63051b3a7

Sell (LinkedIn only): Frobb Media agent orchestration · $5,000/month
Product (Reddit/FB/X): https://retirefire.net

